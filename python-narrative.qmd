...
athlete_data['month'] = athlete_data['bday'].dt.month
athlete_data['day'] = athlete_data['bday'].dt.day
```

I'll confess I'm more used to the `decimal_date` function in R/lubridate, and I'm envisioning being able to just look at a density plot of each sport's birthdays over time (modulo year). 
I didn't find a handy equivalent in pandas or the date/time libraries in Python, so let's build one...

```{python}
def decimal_date(x):
  ns_since_epoch =  pd.to_datetime(pd.to_datetime(x).value)
